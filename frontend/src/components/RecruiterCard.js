...
function buildSummary(profile, repos, languages, events, score) {
  ...
  const expStr   = years >= 2
    ? `${years}-year GitHub developer`
    : years === 1
      ? `GitHub developer since ${formatMonthYear(profile.createdAt)}`
      : 'actively building on GitHub';
  const langStr  = topLangs.length > 0
    ? `primarily working in ${topLangs.join(', ')}`
    : 'working across multiple technologies';
  const headline = `${profile.name} is a ${expStr}, ${langStr}.`;

  const bullets = [];
  if (own.length > 0)
    bullets.push(`${own.length} original repositories across ${languages.length} language${languages.length !== 1 ? 's' : ''}.`);
  if (totalStars >= 10)
    bullets.push(`${totalStars} total stars earned across repositories.`);
  if (topRepo && topRepo.stars > 0)
    bullets.push(`Top project: "${topRepo.name}" (${topRepo.stars} stars)${topRepo.description ? ' — ' + topRepo.description.slice(0, 80) : ''}.`);
  if (d90 >= 3)
    bullets.push(`${d90} repositories pushed in the last 90 days.`);
  else if (events?.totalCommits > 0)
    bullets.push(`${events.totalCommits} commits in the last 90 days.`);
  if (events?.streak >= 3)
    bullets.push(`Current coding streak: ${events.streak} consecutive days.`);
  bullets.push(`Developer score: ${score.total}/100 (${score.label}).`);

  return { headline, bullets };
}

export default function RecruiterCard({ profile, repos, languages, events, score, onCopied }) {
    ...
    <div className="recruiter-card card fade-in">
      <div className="recruiter-header">
        <div>
          ...
          <p className="recruiter-sub">Generated from public GitHub data</p>
        </div>
